...
// Create a new order
const createOrder = async (userId, productId, status) => {
  const query =
    "INSERT INTO orders (user_id, product_ids, status) VALUES ($1, $2, $3) RETURNING id";
  const result = await db.query(query, [userId, productId, status]);
